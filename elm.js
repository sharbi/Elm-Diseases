...
Elm.Circles.make = function (_elm) {
   ...
   sLocs) {
      return function () {
         ...
         var c = $Basics.toFloat($Basics.fst(sLoc)) - 450;
         var d = 500 - $Basics.toFloat($Basics.snd(sLoc));
         var dLoc = A2($Maybe.withDefault,
         {ctor: "_Tuple2",_0: 0,_1: 0},
         A2($Dict.get,dId,dLocs));
         var a = $Basics.toFloat($Basics.fst(dLoc)) - 450;
         var b = 500 - $Basics.toFloat($Basics.snd(dLoc));
   ...
   input) {
      return function () {
         ...
         _L.fromArray([editButton
                      ,$Html.fromElement($Graphics$Element.show(m.dId))
                      ,$Html.fromElement($Graphics$Element.show(m.sId))]))]));
      }();
   });
